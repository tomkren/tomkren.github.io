...
Despite the fact that the geometric strategy was designed for typed GP over lambda trees it works well for standard GP problems satisfying the closure requirement. Furthermore, for such problems our approach generates standard S-expressions only. Another reason for including standard GP benchmarks in our experiments is the fact that the results are seriously affected by a crossover 
operator. Design of a good crossover for typed GP remains an open problem. Our crossover is quite complicated and beyond the scope of this paper, but for problems satisfying the closure requirement it behaves exactly the same as the standard GP crossover. 
...
In order to ensure that the experiments are replicable we chose to use the standard GP system described in \cite{koza92}, with all the parameters set to their default values (which are sumarized in table~I), only with the following exceptions:

\begin{enumerate}
  \item In the second and third experiment the preservation of the best individual into the next generation is performed.
  \item In the third experiment we use our crossover operation briefly mentioned above together with use of "applicative" tree representation.
\end{enumerate}
...
\begin{figure}[!ht]
  ...
  \includegraphics[scale=0.12]{imgs/SSR.eps}
\end{figure}
...
But there is also mentioned one solution, which is described as correct solution. This program needs 545 time steps; if it is given only 400 time steps, then it eats only 79 pieces of food. 
...
\begin{figure}[!ht]
  ...
  \includegraphics[scale=0.13]{imgs/ANT.eps}
\end{figure}
...
\begin{figure}[!ht]
  ...
  \includegraphics[scale=0.12]{imgs/EP.eps}
\end{figure}
...
some modification of \texttt{foldr xor True inputList} one sees that important 
