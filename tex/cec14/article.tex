...
Three well-known benchmark experiments (simple symbolic regression,
artificial ant problem and even parity problem)
...
Despite the fact that the presented system is designed for typed genetic 
programming, when given standard genetic programming problem satisfying
standard closure requirement it produces simple S-expressions and
the novel geometric strategy outperforms the standard system. 
This claim is supported by that first two experiments both satisfying 
...
\red{Přestože je naše generujicí metoda navržena obecně pro typovanou verzi,
tak pracuje dobře i pro problémy standardního GP splňující closure podmínku.
Pro taklovéto problémy strategie generuje standardní S-výrazy. Proto jsou
první dva ze tří experimentů právě takového charakteru. Dalším důvodem pro volbu 
takovýchto problémů je fakt, že výsledky pokusů výrazně ovlivňuje použitý křížící 
operátor. Návrh obecného křížícího operatoru pro GP v symply typed lc je otevřený problém. Protože křížící operátor použitý v našem systému je poměrně komplikovaný
a tím pádem beyond the scope of this paper, ale pro problémy splnující
clousure podmínku se chová přesně stejně jako standardní křížící operator,
tak se to zdá jako další důvod proč 2/3 experimentů udělat na closed problémech.}
...
\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.14]{imgs/cec/SSR.PNG}
...
\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.14]{imgs/cec/ANT.PNG}
...
\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.14]{imgs/cec/EP.PNG}
