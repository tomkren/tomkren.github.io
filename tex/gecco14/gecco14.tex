...
\category{H.4}{\red{TODO}}{Miscellaneous}
%A category including the fourth, optional field follows...
\category{D.2.8}{Software Engineering}{Metrics}[complexity measures, performance measures]

\terms{\red{TODO}}

\keywords{\red{TODO, ToDo, todo}}
...
- Možná někam poznamenat že přínos toho článku je taky v tom že zpřístupňuje pojmy z teorie typovaného lc komunitě typovaného GP.. 
...
\red{TODO zatim jen zkopčený z minula a dodaná diskuze 
zatim česky porovnávající nás a kombinatoři , 
opravit podle novýho kontextu}
...
POZNAMKY PRO DISKUSI POROVNAVAJICI NÁS A BRIGGSE:
...
\red{TODO: ještě určitě projít a vylepšit; někde zestručnit někde rozšířit}
...
\red{asi řít tady v ty subsekci co že to je normalizace termu jednou větou}
...
some another type. Let us list some possible approaches to solving this problem.
...
the \textit{hybrid}, because it can be understood as hybrid of the
lambda calculus representation and the purely combinator representation
used in \cite{kes}. Advantage of generating terms as \lnf lambda terms
...
lambda term as combinator term (this can be seen considering that 
\lnf term $\lamb{x}{x}$ is equivalent to both $\Icomb$ and 
$\Scomb \Kcomb \Kcomb$), but every combinator term has only 
one unique \lnf form. Unfortunately the disadvantage is also important;
...
terms generated directly are probably smaller than those mechanically
translated by the abstraction elimination. 
The act of producing a small normalized term and translating it
immediately to a bulky combinator form seems as a wasteful behavior.

Thus we propose following crossover operator, which we call 
the \textit{unpacking} crossover. Unlike the \textit{hybrid} it operates
over \lterms and produces \lterm offspring in \benf.
...
\red{(TODO: opravdu jí tam diskutovat)}  
The difference between the \textit{unpacking} and \textit{hybrid}
crossover is that in the case of the \textit{unpacking}  
this increase is only temporary stage, which is reversed
by the normalization. 
The most notable disadvantage of this crossover approach is
that it is more time consuming than the simple ones. 
\red{TODO: Až bude známo jak moc to žere čas, tak to sem připsat.
a taky někde zmínit asi výslovně, že pro closed problémy je to 
stejný jako kozovská pač vnich žadný abstrakce nejsou.
Taky nějak vhodně zmínit analogii s přírodou.}
...
Tuhle subsekci ještě pořádně projít, zatim je to tu skopčený z diplomky.

Na tuhle subsekci se odkazuje předchozí text o unpacking s tim že je tu rozebranej vliv
počtu křižicích bodů na kvalitu křížení, takže to tu rozebrat, tzn. musi se odiskutovat
rozdíl mezi @-tree a S-expresion.
